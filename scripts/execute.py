# Based off Google Assistant SDK's pushtotalk.py
...
from valence import load_model, run_model
from actions import play_spotify, pause_spotify
try:
	from . import (
		assistant_helpers,
		audio_helpers,
...
class SampleAssistant(object):
	...
	def assist(self):
		...
		for resp in self.assistant.Assist(self.gen_assist_requests()):
			...
			if resp.event_type == END_OF_UTTERANCE:
				...
				valence = ''
				music_action = ''
				if 'pause' in transcript:
					music_action = 'pause'
				elif 'play' in transcript:
					music_action = 'play'
					valence = run_model(MODEL)
				for resp_f in self.assistant.Assist(
						self.gen_final_request(transcript, valence, music_action)):
					if len(resp_f.audio_out.audio_data) > 0:
						if not self.conversation_stream.playing:
							self.conversation_stream.stop_recording()
							self.conversation_stream.start_playback()
							logging.info('Playing assistant response.')
						self.conversation_stream.write(resp_f.audio_out.audio_data)
						
				if music_action == 'pause':
					pause_spotify()
				elif music_action == 'play':
					play_spotify(valence)
	...
	def gen_assist_requests(self):
		...
		for data in self.conversation_stream:
			self.conversation_stream.write_to_file(data)
...
def main(api_endpoint, credentials, project_id,
	...
	with SampleAssistant(lang, device_model_id, device_id,
		...
		while True:
			conversation_stream.reset_saver()
			assistant.said_keyword = assistant.assist()
