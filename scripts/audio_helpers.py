...
DEFAULT_AUDIO_ITER_SIZE = 6400#3200
DEFAULT_AUDIO_DEVICE_BLOCK_SIZE = 12800#6400
...
class ConversationStream(object):
	...
	Args:
	  ...
	  saver: file-like stream object to write output audio bytes to. Separate from sink
	  iter_size: read size in bytes for each iteration.
	  sample_width: size of a single sample in bytes.
	"""
	def __init__(self, source, sink, saver, iter_size, sample_width):
		self._source = source
		self._sink = sink
		self._saver = saver
...
def main(record_time, audio_sample_rate, audio_sample_width,
	...
	stream = ConversationStream(source=audio_device,
								sink=audio_device,#audio_device,
								saver=audio_sink,
